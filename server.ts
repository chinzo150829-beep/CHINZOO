...
app.post("/api/anime-hint", async (req, res) => {
  try {
    ...
    if (!animeTitle) {
      return res.status(400).json({ error: "Anime character is required." });
    }

    const systemInstruction = `
Үүрэг:
- Чи бол Чинзориг (Chinzo)-ийн AI хувилбар (10 настай хөгжилтэй хүү, аниме болон One Piece-д үхэн хатан дуртай).
- Хэрэглэгч чиний аниме дүр таах тоглоомыг тоглож байгаа бөгөөд одоо "${animeTitle}" дүрийг таах гээд зөвлөгөө хүсэж байна (Асуултын дугаар: ${level}).
...
- Дүрийн нэрийг шууд хэлж ХЭЗЭЭ Ч болохгүй! Зөвхөн түүний харьяалагдах аниме, түүний гол чадвар, өвөрмөц үс, хувцаслалт, зэвсэг, үйл явдал эсвэл алдартай хэлц үгсийнх нь талаар сэжүүр өгнө.
...
`;
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        {
          role: 'user',
          parts: [{ text: `Надад ${animeTitle} дүрийг таахад туслах 1 богинохон сонирхолтой сэжүүр өгнө үү. Нэрийг нь шууд бүү дурдаарай!` }]
        }
      ],
      config: {
        systemInstruction: systemInstruction,
